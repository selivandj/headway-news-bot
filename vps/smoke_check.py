...
def main() -> int:
    ...
    training_service = read_first_existing("headway-news-monitor-training7d.service", "/etc/systemd/system/headway-news-monitor-training7d.service")
    ...
    check("ExecStart=/opt/headway-news-bot/vps/run_training_monitor.sh" in training_service, "7-day training service uses vps run script", errors)
    check("TRAINING_TARGET_DRAFTS_PER_DAY" in training_script, "7-day training monitor keeps daily target setting", errors)
    check('"$BASE_DIR/vps/monitor.py" --hours "$LOOKBACK_HOURS" --send-review' in training_script, "7-day training script runs vps monitor in review mode", errors)
